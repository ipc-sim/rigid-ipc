...
def main(args=[]):
    ...
    parser.add_argument('output_folder',
                        ...
                        nargs='?',
                        default=None,
                        help='folder where to save output(s)')
    ...
    with fin.open("r") as json_file:
        ...
        for i in range(0, len(vertices_sequence)):
            ...
            total_edges = np.append(total_edges,
                                    edges + total_xyz.shape[0],
                                    axis=0)
            total_xyz = np.append(total_xyz, xyz, axis=0)
            total_time_data = np.append(total_time_data,
                                        np.ones(edges.shape[0]) * i)
            total_vertex_data = np.append(
                total_vertex_data,
                np.arange(0, len(xyz)).astype(np.float64))
