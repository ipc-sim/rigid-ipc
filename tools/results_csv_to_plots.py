...
def plot_data(x, ys, labels, fout, with_labels):
    ...
    matplotlib.rcParams.update({
        'font.size': fontsize, 'font.family':'serif', 'font.serif':'Times New Roman'})
    ...
    if (with_labels):
        ...
        ax.legend(prop={'weight':'normal'}, loc='upper left', bbox_to_anchor=(1, 1))
    plt.savefig(fout)

def main(args=[]):
    parser = argparse.ArgumentParser(
        description='Create plot from data')
    parser.add_argument('energy_file', metavar='energy.csv',
                        type=str,  help='configuration for the plots')
    parser.add_argument('output_file', metavar='output.pdf', type=str,
                        default=".", help='fname and format ouf the image')
    parser.add_argument('--max-step',  type=int,
            default=-1, help='final step to include')
    parser.add_argument('--no-labels',  type=int,
            default=-1, help='final step to include')
