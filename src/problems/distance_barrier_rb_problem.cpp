...
namespace opt {
        ...
        const Eigen::VectorXd& sigma)
    {
        ...
        jac_g.resize(constraint_.m_num_constraints, num_vars_);
        jac_g.setZero();
        ...
        for (size_t i = 0; i < constraint_.m_ev_distance_active.size(); ++i) {
            ...
            size_t cstr_id = constraint_.m_ev_distance_active_map[i];
        ...
        if (!compare_jacobian(jac_g, jac_g_approx)) {
            eval_jac_g_approx(*this, sigma);
            spdlog::error(
                "rigid_body status=fail "
    ...
    {
        std::vector<Eigen::SparseMatrix<double>> gx_hessian;
        gx_hessian.resize(size_t(constraint_.m_num_constraints));
        ...
        Eigen::SparseMatrix<double> global_el_hessian(num_vars_, num_vars_);
        for (size_t i : constraint_.m_ev_impact_active_map) {
            gx_hessian[i] = global_el_hessian;
        }
        for (size_t i : constraint_.m_ev_inactive_map) {
            gx_hessian[i] = global_el_hessian;
        }
