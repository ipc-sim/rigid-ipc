...
bool update_selection(
    ...
    const Eigen::RowVector3d& click_coord, const double thr, std::vector<int>& selection)
{
...
void translation_delta(
    const Eigen::MatrixXd& points, const int modifier,
    const Eigen::RowVector3d& click_coord, const std::vector<int>& selection, Eigen::RowVector3d& delta)
{
    size_t num_sl = selection.size();
    assert(num_sl > 0);

    Eigen::RowVector3d current = points.row(selection[num_sl - 1]);
...
bool ViewerMenu::mouse_down(int button, int modifier)
{
    ...
    if (igl::unproject_onto_mesh(Eigen::Vector2f(x, y),
        ...
        clicked_on_canvas(button, modifier, coords);
    }
    return true;
}
...
void ViewerMenu::update_vector_field(const unsigned long data_id, const Eigen::MatrixXd& x0, const Eigen::MatrixXd& delta)
{
    ...
    for (unsigned i = 0; i < lines.rows(); ++i) {
        lines.row(i).segment(0, 6) << x0.row(i), x1.row(i);
    }
    for (unsigned i = 0; i < points.rows(); ++i) {
        points.row(i).segment(0, 3) << x1.row(i);
    }
...
void ViewerMenu::update_graph(const unsigned long data_id, const Eigen::MatrixXd& vertices, const Eigen::MatrixXi& edges)
{
    ...
    Eigen::MatrixXd& points = viewer->data_list[data_id].points;
    for (unsigned i = 0; i < points.rows(); ++i) {
        points.row(i).segment(0, 3) = vertices.row(i);
    }
    Eigen::MatrixXd& lines = viewer->data_list[data_id].lines;
    for (unsigned i = 0; i < edges.rows(); ++i) {
        lines.row(i).segment(0, 6) << vertices.row(edges(i, 0)), vertices.row(edges(i, 1));
    }
