    ...
    Eigen::VectorXd& grad)
{
    ...
    if (edge_id == impact->impacted_edge_index) {
        ...
        impact_node = impact->impacting_alpha < 0.5 ? ccd::autodiff::vK : ccd::autodiff::vL;

    } else if (edge_id == impact->impacting_edge_index) {
        e_kl = edges.row(impact->impacted_edge_index);
        impact_node = impact->impacting_alpha < 0.5 ? ccd::autodiff::vI : ccd::autodiff::vJ;
    } else {
        return;
    }
    ...
    Vector8d el_grad = ccd::autodiff::collision_volume_grad(Vi, Vj, Vk, Vl, Ui, Uj, Uk, Ul, impact_node, epsilon);
