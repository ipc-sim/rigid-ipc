...
{
    PROFILE_POINT("rigid_collisions_detection__narrow_phase");
    PROFILE_START();

    auto detect_ev_collision =
        [&](const ipc::EdgeVertexCandidate& ev_candidate) {
            double toi, alpha;
            bool is_colliding = edge_vertex_rigid_ccd(
    ...
    auto detect_ee_collision = [&](const ipc::EdgeEdgeCandidate& ee_candidate) {
        ...
        bool is_colliding = edge_edge_rigid_ccd(
    ...
    auto detect_fv_collision =
        [&](const ipc::FaceVertexCandidate& fv_candidate) {
            double toi, u, v;
            bool is_colliding = face_vertex_rigid_ccd(
