...
public:
    ...
    inline int minimize(Foo& f, Vector& x, Scalar& fx,
        std::function<bool(const Eigen::VectorXd& x)> callback)
    {
        const int n = x.size();
        ...
        for (;;) {
            ...
            try {
                LineSearch<Scalar>::LineSearch(
                    f, fx, x, m_grad, step, m_drt, m_xp, m_param);
            } catch (std::runtime_error err) {
                spdlog::warn(
                    "solver=BFGS iter={:d} failure=\"{:s}\"", k, err.what());
                if (k != 1) {
                    return minimize(f, x, fx, callback);
                }
                throw err;
            }
