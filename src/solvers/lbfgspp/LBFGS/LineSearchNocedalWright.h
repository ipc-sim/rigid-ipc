        ...
        const LBFGSParam<Scalar>& param)
    {
        ...
        if (param.linesearch != LBFGS_LINESEARCH_BACKTRACKING_STRONG_WOLFE)
            throw std::invalid_argument("'param.linesearch' must be "
                                        "'LBFGS_LINESEARCH_BACKTRACKING_STRONG_"
                                        "WOLFE' for LineSearchNocedalWright");
        ...
        //
        //   See also:
        //     "Numerical Optimization", "Algorithm 3.5 (Line Search
        //     Algorithm)".
        int iter = 0;
        for (;;) {
            ...
            if (fx - fx_init > step * dg_test || fx >= fx_lo) {
                if (step == step_hi)
                    throw std::runtime_error(
                        "the line search routine failed, possibly due to "
