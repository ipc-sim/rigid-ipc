...
namespace ccd {
void regular_2d_grid(const int n, const bool tri, Eigen::MatrixXd &V, Eigen::MatrixXi &F)
{
...
void regular_2d_grid(const int num_cols, const int num_rows, Eigen::MatrixXd &V, Eigen::MatrixXi &F)
{
        ...
        for (int i = 0; i < num_cols - 1; ++i)
        {
                ...
                {
                        tmp = {{map[i + j * num_cols], map[i + 1 + j * num_cols], map[i + (j + 1) * num_cols]}};
                        if (tmp[0] >= 0 && tmp[1] >= 0 && tmp[2] >= 0)
                        {
                                F.row(index) << tmp[0], tmp[1], tmp[2];
                                ++index;
                        ...
                        tmp = {{map[i + 1 + j * num_cols], map[i + 1 + (j + 1) * num_cols], map[i + (j + 1) * num_cols]}};
                        if (tmp[0] >= 0 && tmp[1] >= 0 && tmp[2] >= 0)
                        {
                                F.row(index) << tmp[0], tmp[1], tmp[2];
                                ++index;
